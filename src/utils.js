...
Logdown.disable(
  "messenger*",
  "-messenger-error",
  "-messenger-warn" /*, '-messenger-info', '-messenger-log'*/
);
...
const Utils = {
  ...
  getCurrentExtensionPart: function() {
    ...
    if (typeof chrome !== "undefined") {
      ...
      if (browser.devtools) {
        ...
        retVal =
          backgroundPage === window ? Constants.BACKGROUND : Constants.POPUP;
      } else {
        retVal = Constants.CONTENT_SCRIPT;
      }
    } else {
      loggerError.error(
        "Could not identify extension part... are you running in a chrome extension context?"
      );
    }
  ...
  removeMessengerPortNamePrefix: function(portName) {
    return portName.replace(
      new RegExp("^" + Constants.MESSENGER_PORT_NAME_PREFIX),
      ""
    );
  }
};
