  ...
  switch (this._myExtPart) {
    ...
    case Constants.DEVTOOL: {
      let doInitConnection = function(tabId) {
        Utils.log(
          "log",
          "[Connection:_init]",
          "Attempting connection initing..."
        );

        this._port =
          this._myExtPart === Constants.BACKGROUND
            ? new MockPort({ name: this._myName })
            : browser.runtime.connect({ name: this._myName });
      ...
      switch (this._myExtPart) {
        ...
        case Constants.POPUP:
          browser.tabs
            ...
            .then(tabs => doInitConnection(tabs[0].id));
    ...
    default: {
      Utils.log(
        "error",
        "[Connection:_init]",
        "Unknown extension part: " + extPart
      );
    }
  }
};
...
Connection.prototype._attemptDeadCbCleanup = function() {
  if (Object.keys(this._pendingCb).length > PENDING_CB_SIZE_CLEANUP_TRIGGER) {
    Utils.log(
      "log",
      "[Connection:_attemptDeadCbCleanup]",
      "Attempting dead callback cleaning... current callbacks number:".Object.keys(
        this._pendingCb
      ).length
    );

    let cleanUpToIndex =
      this._pendingCbCleanupIndex + PENDING_CB_SIZE_CLEANUP_AMOUNT;
    ...
    Utils.log(
      "log",
      "[Connection:_attemptDeadCbCleanup]",
      "New callbacks number after cleaning done:",
      Object.keys(this._pendingCb).length
    );
  }
};
  ...
  return new Promise(resolve => {
    ...
    switch (this._myExtPart) {
      ...
      case Constants.POPUP: {
        browser.tabs
          .query({ active: true, currentWindow: true })
          .then(tabs => (message.tabId = tabs[0].id));
...
Connection.prototype._postResponse = function(
  fromPort,
  responseValue,
  origMessage
) {
...
Connection.prototype._handleMessage = function(message, fromPort) {
  ...
  let fromTabId =
    message.fromTabId && message.from !== Constants.BACKGROUND
      ? `:${message.fromTabId}`
      : null;
  let from = `${message.from}:${fromName}` + (fromTabId ? `:${fromTabId}` : "");

  // Invoke the user message handler.
  this._userMessageHandler(
    message.userMessage,
    from,
    message.fromPortSender,
    sendResponse
  );
};
...
Connection.prototype._handleResponse = function(response) {
  if (this._pendingCb[response.cbId]) {
    ...
    Utils.log(
      ...
      "Ignoring response sending because callback does not exist (probably already been called)"
    );
  }
};

Connection.prototype._sendMessage = function(
  port,
  toExtPart,
  toNames,
  toTabId,
...
Connection.prototype._addMessengerPortNamePrefix = function(toNames) {
  return toNames.map(function(toName) {
    ...
    return toName === Constants.TO_NAME_WILDCARD
      ? toName
      : Constants.MESSENGER_PORT_NAME_PREFIX + toName;
  });
};
...
Connection.prototype._validateMessage = function(toExtPart, toName, toTabId) {
  ...
  if (
    toExtPart !== Constants.BACKGROUND &&
    toExtPart !== Constants.CONTENT_SCRIPT &&
    toExtPart !== Constants.DEVTOOL &&
    toExtPart !== Constants.POPUP
  ) {
    return (
      'Unknown extension part in "to" argument: ' +
      toExtPart +
      "\nSupported parts are: " +
      Constants.BACKGROUND +
      ", " +
      Constants.CONTENT_SCRIPT +
      ", " +
      Constants.POPUP +
      ", " +
      Constants.DEVTOOL
...
Connection.prototype._onPortMessageHandler = function(message, fromPort) {
  switch (message.type) {
    case Constants.INIT_SUCCESS: {
      ...
      this._pendingInitMessages.forEach(
        function(pendingInitMessage) {
    ...
    case Constants.RESPONSE: {
      if (!message.to) {
        Utils.log(
          "error",
          "[Connection:_onPortMessageHandler]",
          'Missing "to" in message: ',
          message
        );
      }
      if (!message.toNames) {
  ...
  return new Promise((cbPromiseResolve, reject) => {
    ...
    if (!this._port) {
      Utils.log(
        ...
        "Rejecting sendMessage because connection does not exist anymore"
      );
      return reject(
        new Error(
          "Connection port does not exist anymore, did you disconnect it?"
        )
      );
    }
    ...
    try {
      ...
      Utils.log(
        "error",
        "[Connection:sendMessage]",
        'Invalid format given in "to" argument: ' + to,
        arguments
      );
    }
    ...
    if (errorMsg) {
      Utils.log("error", "[Connection:sendMessage]", errorMsg, arguments);
    }
