  ...
  switch (message.type) {
    ...
    case Constants.RESPONSE: {
      ...
      if (!message.to) {
        Utils.log(
          "error",
          "[BackgroundHub:_onPortMessageHandler]",
...
BackgroundHub.prototype._getPortsObj = function(extensionPart) {
  switch (extensionPart) {
    ...
    default:
      Utils.log(
        "error",
        "[BackgroundHub:_onPortDisconnectionHandler]",
        "Unknown extension part: " + extensionPart
      );
  }
};
...
BackgroundHub.prototype._relayMessage = function(message, fromPort) {
  ...
  if (from === Constants.BACKGROUND) {
    ...
    Utils.log(
      "error",
      "[BackgroundHub:_relayMessage]",
      'Unknown "from" in message: ' + from
    );
  }
  ...
  let toPorts;
  if (to === Constants.BACKGROUND) {
    toPorts = this._backgroundPorts[BACKGROUND_PORT_UID_KEY]
      ? this._backgroundPorts[BACKGROUND_PORT_UID_KEY]
      : [];
    ...
    toPorts = this._contentScriptPorts[tabId]
      ? this._contentScriptPorts[tabId]
      : [];
  } else {
    Utils.log(
      "error",
      "[BackgroundHub:_relayMessage]",
      'Unknown "to" in message: ' + to
    );
  }
  ...
  if (toPorts.length === 0) {
    Utils.log(
      "info",
      "[BackgroundHub:_relayMessage]",
      'Not sending relay because "to" port does not exist'
    );
  }
    ...
    function(toName) {
      ...
      if (matchedPorts.length > 0) {
        ...
        Utils.log(
          "warn",
          "[BackgroundHub:_relayMessage]",
          "Could not find any connections with this name (probably no such name):",
          Utils.removeMessengerPortNamePrefix(toName)
        );
      }
    }.bind(this)
  );
  ...
  // Send the message/s.
  matchingToPorts.forEach(
    function(matchingToPort) {
      matchingToPort.postMessage(message);
    }.bind(this)
  );
};

BackgroundHub.prototype._onPortDisconnectionHandler = function(
  disconnectedPort
) {
    ...
    for (let i = 0; i < portKeys.length; i++) {
      ...
      for (var j = portsArrLength; j >= 0; j--) {
        ...
        if (port === disconnectedPort) {
          Utils.log(
            "log",
            "[BackgroundHub:_onPortDisconnectionHandler]",
            "Remove connection of port with unique id: ",
            currPortKey
          );
          portsArr.splice(j, 1);
          ...
          if (this._disconnectedHandler) {
            ...
            let tabId =
              extensionPart !== Constants.BACKGROUND
                ? parseInt(currPortKey)
                : null;
            let userPortName = Utils.removeMessengerPortNamePrefix(
              disconnectedPort.name
            );
      ...
      if (portsObj[currPortKey].length === 0) {
        Utils.log(
          "log",
          "[BackgroundHub:_onPortDisconnectionHandler]",
          "Removing empty ports object for unique id: ",
          currPortKey
        );
